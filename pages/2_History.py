...
if not all_runs:
    st.info("No runs found in the database. Please start a new run from the other pages."); st.stop()
...
if search_query:
    query = search_query.strip(); filtered_df = filtered_df[filtered_df['run_id'].str.contains(query, case=False, na=False) | filtered_df['model_name'].str.contains(query, case=False, na=False) | filtered_df['dataset_name'].str.contains(query, case=False, na=False)]
...
st.markdown("---")
header_cols = st.columns([1, 3, 2, 1, 2, 1]); header_cols[0].write("**Type**"); header_cols[1].write("**Model**"); header_cols[2].write("**Dataset**"); header_cols[3].write("**Status**"); header_cols[4].write("**Start Time**"); header_cols[5].write("**Actions**")

for index, row in paginated_df.iterrows():
    col1, col2, col3, col4, col5, col6 = st.columns([1, 3, 2, 1, 2, 1]); col1.markdown(f"`{row['run_type']}`"); col2.write(row['model_name']); col3.write(row['dataset_name'])
    status_color = "green" if row['status'] == 'COMPLETED' else "orange" if row['status'] in ['STARTED', 'ABORTED'] else "red"; col4.markdown(f":{status_color}[{row['status']}]")
        ...
        if run_details:
            ...
            with st.expander("📊 Performance Metrics", expanded=True):
                if p_metrics:
                    if "overall" in p_metrics:
                        ...
                        c1.metric("Accuracy", f"{overall.get('accuracy', 0):.4f}"); c2.metric("Precision", f"{overall.get('precision', 0):.4f}"); c3.metric("Recall", f"{overall.get('recall', 0):.4f}"); c4.metric("F1-Score", f"{overall.get('f1_score', 0):.4f}")
                    st.divider()
                    st.subheader("Processing Speed")
                    timing_cols = st.columns(2)
                    if "total_rows" in p_metrics: # Inference specific
                        timing_cols[0].metric("Total Time", f"{p_metrics.get('total_time_sec', 0):.2f}s")
                        timing_cols[1].metric("Time / Record", f"{p_metrics.get('time_per_prediction_ms', 0):.2f} ms")
                    elif "total_run_time_sec" in p_metrics.get("overall", {}): # Training / Testing
                        timing_cols[0].metric("Total Run Time", f"{p_metrics['overall'].get('total_run_time_sec', 0):.2f}s")
                        tpr_ms = p_metrics['overall'].get('time_per_record_ms') or p_metrics['overall'].get('time_per_eval_record_ms', 0)
                        timing_cols[1].metric("Time / Record (eval)", f"{tpr_ms:.2f} ms")
            ...
            with st.expander("🛠️ Resource Metrics", expanded=True):
                if r_metrics and 'summary' in r_metrics:
                    ...
                    if gpu: st.divider(); st.subheader("GPU"); gpu_cols = st.columns(4); gpu_cols[0].metric("Avg GPU Util", f"{gpu.get('avg_gpu_util_percent', 0):.2f}%"); gpu_cols[1].metric("Avg VRAM", f"{gpu.get('avg_gpu_mem_gb', 0):.2f} GB", help=f"Total: {gpu.get('total_gpu_mem_gb', 0):.2f} GB"); gpu_cols[2].metric("Avg Power", f"{gpu.get('avg_power_watts', 0):.2f}W"); gpu_cols[3].metric("Avg Clock", f"{gpu.get('avg_clock_mhz', 0):.0f} MHz")
            ...
            with st.expander("📄 Reports & Visualizations", expanded=True):
                ...
                if report_path_str and Path(report_path_str).exists():
                    ...
                    if report_dir.is_dir():
                        ...
                        image_paths = [str(report_dir / f) for f in plot_files if (report_dir / f).exists()]
                        if image_paths:
                            plot_cols = st.columns(2)
                            for i, path in enumerate(image_paths):
                                plot_cols[i % 2].image(path, use_column_width=True)
                        else: st.info("No plot images found in the report directory.")
                    elif report_dir.is_file() and report_dir.suffix == '.csv':
                         with open(report_dir, "rb") as fp: st.download_button("Download Results CSV", fp, report_dir.name, "text/csv")
