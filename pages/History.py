...
if selected_option:
    ...
    if details:
        ...
        hardware_metrics = {
            "Total Run Time": f"{details.get('total_run_time_sec', 0):.2f}s",
            "Training Time": f"{details.get('training_time_sec', 0):.2f}s",
            "Testing Time": f"{details.get('testing_time_sec', 0):.2f}s",
            "Avg RAM Usage": f"{details.get('avg_ram_usage_gb', 0):.2f} GB",
            "95th Peak RAM": f"{details.get('peak_95_ram_usage_gb', 0):.2f} GB",
            "Avg VRAM Usage": f"{details.get('avg_gpu_vram_gb', 0):.2f} GB",
            "95th Peak VRAM": f"{details.get('peak_95_gpu_vram_gb', 0):.2f} GB",
        }
        ...
        with col1:
            ...
            st.metric("F1-Score", f"{details.get('f1_score', 0):.4f}")
        ...
        with col2:
            ...
            st.metric("Total Time", f"{details.get('total_run_time_sec', 0):.2f}s")
