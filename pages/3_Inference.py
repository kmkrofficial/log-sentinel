...
def reset_global_state():
    GLOBAL_APP_STATE["is_task_running"] = False; GLOBAL_APP_STATE["task_type"] = None
    GLOBAL_APP_STATE["log_buffer"] = []; GLOBAL_APP_STATE["latest_progress"] = {}
    GLOBAL_APP_STATE["stop_requested"] = False; GLOBAL_APP_STATE["result_buffer"] = None
    GLOBAL_APP_STATE["error"] = None; GLOBAL_APP_STATE["done"] = False
...
def run_batch_inference_in_thread(selected_run_id, uploaded_file):
    controller, db_manager, visualizer = None, None, None
    run_id = None
    with APP_LOCK:
        if GLOBAL_APP_STATE.get("is_task_running"):
            GLOBAL_APP_STATE["error"] = "Another task is already running."; return
        reset_global_state()
        GLOBAL_APP_STATE["is_task_running"] = True; GLOBAL_APP_STATE["task_type"] = "Inference"
    try:
        db_manager = DatabaseManager(DB_PATH)
        run_id = db_manager.create_new_run('Inference', f"Inference on {selected_run_id[:8]}...", uploaded_file.name, None)
        ...
        output_file_path, perf_metrics, class_data = controller.predict_batch(uploaded_file)
        
        final_status = 'FAILED'
        if output_file_path and perf_metrics:
            GLOBAL_APP_STATE["result_buffer"] = (output_file_path, perf_metrics)
            db_manager.save_performance_metrics(run_id, perf_metrics)
            final_status = 'COMPLETED'
            
            if class_data:
                gt, pred, proba = class_data
                p, r, f1, _ = precision_recall_fscore_support(gt, pred, average='binary', pos_label=1, zero_division=0)
                class_metrics = {"overall": {"accuracy": accuracy_score(gt, pred), "precision": p, "recall": r, "f1_score": f1}}
                db_manager.save_performance_metrics(run_id, {**perf_metrics, **class_metrics}) # Merge metrics
                
                visualizer.plot_confusion_matrix(confusion_matrix(gt, pred), ['Normal', 'Anomalous'])
                visualizer.plot_roc_curve(gt, proba)

        elif GLOBAL_APP_STATE["stop_requested"]: final_status = 'ABORTED'
        
        resource_metrics = monitor.stop()
        visualizer.plot_resource_usage(resource_metrics) # Visualize resource usage for inference
        db_manager.save_resource_metrics(run_id, resource_metrics)
        report_path = str(report_dir) if final_status == 'COMPLETED' else None
        db_manager.update_run_status(run_id, final_status, report_path)
        
    except Exception as e:
        print(f"Error during batch inference thread: {e}"); GLOBAL_APP_STATE["error"] = str(e)
    finally:
...
with col1:
    ...
    if inference_mode == "Single Prediction":
        with st.container(border=True):
            st.subheader("Enter Log Sequence")
            log_sequence_input = st.text_area("Use ' ;-; ' to separate log lines", height=150, disabled=is_any_task_running)
        ...
        with st.container(border=True):
            ...
            st.info("File must contain a 'Content' column. If 'Label' column exists, classification metrics will be generated.")
            ...
            if st.button("Start Batch Inference", use_container_width=True, disabled=is_any_task_running):
                if not selected_run_id or not uploaded_file: st.warning("Please select a model and upload a file.")
                    ...
                    thread = threading.Thread(target=run_batch_inference_in_thread, args=(selected_run_id, uploaded_file)); thread.start(); st.rerun()
...
with col2:
    ...
    if is_this_task_running:
        ...
        progress = progress_info.get("progress", 0); rows = progress_info.get("rows_processed", 0); etc = progress_info.get("etc", 0)
