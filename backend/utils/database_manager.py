...
class DatabaseManager:
    ...
    def create_new_run(self, run_type, model_name, dataset_name, hyperparameters, nickname, status='RUNNING'):
        with self._get_conn() as conn:
            cursor = conn.cursor()
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            ''', (run_type, model_name, dataset_name, hp_json, nickname, status))
            conn.commit()
    ...
    def get_run_by_nickname(self, nickname):
        with self._get_conn() as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM runs WHERE nickname = ?", (nickname,))
            row = cursor.fetchone()
            if row:
                details = dict(row)
                details['hyperparameters'] = json.loads(details.get('hyperparameters', '{}'))
