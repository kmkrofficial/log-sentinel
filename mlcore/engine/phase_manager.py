...
from mlcore.engine.data_utils import BalancedSampler, FocalLoss
from mlcore.utils.helpers import format_time
...
def evaluate(controller: 'TrainingController', dataset, dataset_name, epoch=-1, collect_outputs=False):
    ...
    with torch.no_grad():
        for i, (sequences, labels) in enumerate(loader):
            if controller.is_gui_mode and i % 10 == 0:
                controller._emit_callback({"status": f"Evaluating {dataset_name}: Batch {i+1}/{len(loader)}"})
    ...
    if controller.callback and epoch != -1: controller._emit_callback({"validation_metrics": metrics})
    ...
    if collect_outputs:
        ...
        return metrics, eval_time, evaluation_outputs
...
def evaluate_and_visualize(controller: 'TrainingController', dataset, dataset_name):
    ...
    metrics, eval_time, evaluation_outputs = evaluate(controller, dataset, dataset_name, epoch=999, collect_outputs=True)
    ...
    controller.run_metrics['training_loss'] = controller._to_serializable(controller.batch_losses)
    controller.run_metrics['evaluation'][dataset_name] = {
        'metrics': metrics,
        'all_labels': controller._to_serializable(evaluation_outputs['all_labels']),
        'all_preds': controller._to_serializable(evaluation_outputs['all_preds']),
        'all_probs': controller._to_serializable(evaluation_outputs['all_probs']),
    }
    controller._write_run_metrics()
