...
def evaluate_and_visualize(controller: 'TrainingController', dataset, dataset_name):
    ...
    metrics, eval_time = evaluate(controller, dataset, dataset_name, epoch=999) # Use the updated evaluate function
    metrics.update({f'{dataset_name}_inference_time_sec': eval_time, f'{dataset_name}_samples_per_sec': len(dataset) / eval_time})
