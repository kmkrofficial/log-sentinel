...
class TrainingController:
    def __init__(self, model_name, dataset_name, hyperparameters, db_manager, callback=None, nickname=None):
    ...
    def _initialize_run(self):
        self.run_id = self.db.create_new_run('Training', self.model_name, self.dataset_name, self.hp, self.nickname)
        if self.run_id:
            self._log(f"Created new training run with ID: {self.run_id} (Nickname: {self.nickname})")
    ...
    def _train_phase(self, phase_name, n_epochs, lr, indexes_for_phase):
        ...
        self._log(f"\n--- Starting Training Phase: {phase_name} ---"); self.model.train()
        criterion = torch.nn.CrossEntropyLoss() # Reverted to standard unweighted loss
        ...
        model_dtype = next(self.model.parameters()).dtype; use_scaler = (model_dtype == torch.float16)
        ...
        for epoch in range(int(n_epochs)):
            epoch_str = f"Epoch {epoch + 1}/{int(n_epochs)} ({phase_name})"; self._log(f"--- {epoch_str} ---")
            random.shuffle(indexes_for_phase); optimizer.zero_grad()
            for i_th, start_idx in enumerate(range(0, len(indexes_for_phase), self.hp['micro_batch_size'])):
                ...
                current_loss = loss.item() * grad_accum_steps; self.batch_losses.append(current_loss)
    ...
    def _evaluate(self):
        self._log("\n--- Starting Final Evaluation ---"); self.model.eval()
        all_preds, gt_labels = [], self.test_dataset.get_all_labels(); eval_start_time = time.time()
        model_dtype = next(self.model.parameters()).dtype
        autocast_dtype = model_dtype if model_dtype in [torch.float16, torch.bfloat16] else torch.float16
        total_items = len(self.test_dataset)
        with torch.no_grad():
            for i in tqdm(range(0, total_items, self.hp['batch_size']), desc="Evaluating"):
                progress = (i + 1) / total_items; self.callback({"epoch": "Final Evaluation", "progress": progress})
        ...
        perf_metrics = {
            "overall": { "accuracy": accuracy_score(gt_numeric, preds_numeric), "precision": p, "recall": r, "f1_score": f1, "time_per_record_ms": time_per_record_ms },
            "per_class": { "normal": {"precision": p_det[0], "recall": r_det[0], "f1": f1_det[0], "support": int(s_det[0])}, "anomalous": {"precision": p_det[1], "recall": r_det[1], "f1": f1_det[1], "support": int(s_det[1])} },
            "confusion_matrix": confusion_matrix(gt_numeric, preds_numeric, labels=[0, 1]).tolist(), "training_loss_series": self.batch_losses
        }
        ...
        self.visualizer.plot_overall_metrics(metrics_for_plot); self.visualizer.plot_training_loss(self.batch_losses)
        return perf_metrics
    ...
    def run(self):
        ...
        try:
            ...
            for name, setup_func, epochs, lr in training_phases:
                setup_func()
                # Pass a None for weights to use standard CrossEntropyLoss
            ...
            if all_phases_completed:
                ...
                final_model_path = self.report_dir / 'final_model'; self.model.save_ft_model(str(final_model_path))
                final_status = 'COMPLETED'
        except Exception as e:
            error_msg = f"CRITICAL ERROR in run {self.run_id}: {e}"; self._log(error_msg); self.callback({"error": str(e)}); final_status = 'FAILED'
        finally:
