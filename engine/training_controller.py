...
class TrainingController:
    ...
    def _load_and_embed_dataset(self, dataset_path, encoder_model, encoder_tokenizer, progress_state):
    ...
    def run(self):
        ...
        try:
            ...
            train_dataset = self._load_and_embed_dataset(train_dataset_path, encoder_model, encoder_tokenizer, progress_state)
            validation_dataset = self._load_and_embed_dataset(val_dataset_path, encoder_model, encoder_tokenizer, progress_state) if val_dataset_path.exists() else None
            test_dataset = self._load_and_embed_dataset(test_dataset_path, encoder_model, encoder_tokenizer, progress_state) if test_dataset_path.exists() else None
            ...
            if self.run_id:
                try:
                    if resource_metrics_history:
                        df = pd.DataFrame(resource_metrics_history)
                        avg_ram = df['ram_used_gb'].mean()
                        peak_95_ram = df['ram_used_gb'].quantile(0.95)
                        avg_gpu = df['gpu_vram_used_gb'].mean() if 'gpu_vram_used_gb' in df.columns else 0
                        peak_95_gpu = df['gpu_vram_used_gb'].quantile(0.95) if 'gpu_vram_used_gb' in df.columns else 0
                        ...
                        db_metrics = {
                            ...
                            "peak_95_ram_usage_gb": peak_95_ram,
                            "avg_gpu_vram_gb": avg_gpu,
                            "peak_95_gpu_vram_gb": peak_95_gpu
                        }
                        
                        self.db.save_final_metrics(self.run_id, db_metrics)
                        
                        if self.visualizer: 
                            self.visualizer.plot_resource_usage(resource_metrics_history)
                    else:
                        self._log("No resource metrics recorded due to early crash.")
