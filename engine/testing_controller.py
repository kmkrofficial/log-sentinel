...
class TestingController:
    ...
    def _initialize_run(self):
        ...
        if self.run_id:
            self._log(f"Created new test run with ID: {self.run_id}"); self.report_dir = REPORTS_DIR / self.run_id
    ...
    def run(self):
        ...
        try:
            ...
            model_path = str(REPORTS_DIR / self.trained_run_id / 'final_model'); model_name = self.db.get_run_details(self.trained_run_id)['run_info']['model_name']
            ...
            with torch.no_grad():
                for i in tqdm(range(0, total_rows, batch_size), desc="Testing"):
                    ...
                    probas = torch.softmax(logits, dim=-1); all_preds.extend(torch.argmax(probas, dim=-1).cpu().numpy()); all_probas.extend(probas[:, 1].cpu().numpy())
                    progress, elapsed = (i + len(seqs)) / total_rows, time.time() - run_start_time; etc = (elapsed / progress) * (1 - progress) if progress > 0 else 0
                    if self.callback({"progress": progress, "rows_processed": i + len(seqs), "etc": etc}) == 'STOP':
            ...
            self.visualizer.plot_roc_curve(gt_numeric, probas_numeric); final_status = 'COMPLETED'
